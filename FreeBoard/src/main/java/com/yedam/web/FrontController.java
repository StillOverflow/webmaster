...
//대표 컨트롤러 => FrontController : 수많은 뷰에서 들어오는 요청들을 하나의 대표 컨트롤러에서 일괄적으로 처리하는 디자인패턴
...
public class FrontController extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		//회원등록 1)등록화면 2)등록처리 --반드시 순서대로 실행(.jsp 바로연결 X)
...
//		System.out.println("service호출");
		...
		Control control = map.get(page); //page 문자열키의 값(무엇을 실행할지)을 control에 가져옴.
